...
public class GcrsTrendServlet extends MeasurementServlet {
   
    protected Vector getResult(HttpServletRequest request){
        
        Vector result = new Vector();        
        ImmMeasureDao dao = new ImmMeasureDao();
        result.add(dao.getTrend("GCRS-LIS%", "MINIBAR_PROCESSING", -1));
        result.add(dao.getTrend("GCRS-LIS%", "SOURCE_PROCESSING", -1));
        result.add(dao.getTrend("GCRS-LIS%", "MINIBAR_PROCESSING", 0));
        result.add(dao.getTrend("GCRS-LIS%", "SOURCE_PROCESSING", 0));
        return result;        
    }
